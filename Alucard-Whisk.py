...
ctypes.windll.kernel32.SetConsoleTitleW(f'[Alucard Selfbot v{SELFBOT.__version__} - Whisk Edit v{SELFBOT.__editversion__}] | Loading...')
                     ...
                     ░ ▓░▒ ▒   ▒ ░░▒░▒░▓  ▒ ▒▓▒ ▒ ░▒ ▒▒ ▓▒    ░░ ▒░ ░ ▒▒▓  ▒ ░▓    ▒ ░░   
                       ...
                       ░   ░   ░  ░░ ░ ▒ ░░  ░  ░  ░ ░░ ░        ░    ░ ░  ░  ▒ ░  ░      
                         ░     ░  ░  ░ ░        ░  ░  ░          ░  ░   ░     ░           
                                        ...
                                        {Fore.MAGENTA}╭—————————————————————————————————╮
                                        {Fore.MAGENTA}│  Alucard {SELFBOT.__version__} -- Whisk Edit v{SELFBOT.__editversion__} │
                                        {Fore.MAGENTA}│        {Fore.YELLOW}github.com/wh7sk         {Fore.MAGENTA}│
                                        {Fore.MAGENTA}╰————————————————┬————————————————╯
                                        {Fore.MAGENTA}            User │ {Fore.YELLOW}{Alucard.user.name}#{Alucard.user.discriminator} 
                                        {Fore.MAGENTA}              ID │ {Fore.YELLOW}{Alucard.user.id}
                                        ...
                                        {Fore.MAGENTA}          Prefix │ {Fore.YELLOW}{prefix}
                                        {Fore.MAGENTA}       ┈┈┈┈┈┈┈┄╌╶┴╴╌┄┈┈┈┈┈┈
    '''+Fore.RESET)
    ctypes.windll.kernel32.SetConsoleTitleW(f'[Alucard Selfbot v{SELFBOT.__version__} - Whisk Edit v{SELFBOT.__editversion__}] | {Alucard.user.name}#{Alucard.user.discriminator}')
    headers = {
      'Authorization': token,
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f'https://api-ssl.bitly.com/v3/shorten?longUrl={link}&domain=bit.ly&format=json&access_token={bitly_key}') as req:
                    ...
                    r = json.loads(r) 
            new = r['data']['url']
            em = discord.Embed(color=10232053)
...
@Alucard.command()
async def kitten(ctx): # b'\xfc'
    await ctx.message.delete()
    r = requests.get('https://ghoapi.azureedge.net/api/Dimension/').json()
    em = discord.Embed(color=10232053)
    em.set_image(url=r["message"])
    try:
        await ctx.send(embed=em)
    except:
...
@Alucard.command()
async def revav(ctx, member: discord.User=None): # b'\xfc'
    await ctx.message.delete()
    if member is None:
        user = ctx.author
    try:
...
async def av(ctx, *, member: discord.User=None): # b'\xfc'
    await ctx.message.delete()
    format = "gif"
    member = member or ctx.author
    if member.is_avatar_animated() != True:
	    format = "png"
    avatar = member.avatar_url_as(format = format if format != "gif" else None)
    async with aiohttp.ClientSession() as session:
...
async def whois(ctx, *, user: discord.User = None): # b'\xfc'
    ...
    em = discord.Embed(description=user.mention, color=0x8400ff)
...
async def dick(ctx, *, member: discord.User = None): # b'\xfc'
    await ctx.message.delete()
    if member is None:
        user = ctx.author
...
async def hypesquad(ctx, house): # b'\xfc'
    ...
    try:
        request.post('https://canary.discord.com/api/v9/hypesquad/online', headers=headers, json=payload, timeout=10)
    except Exception as e:
        print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)
...
async def tokenfuck(ctx, _token): # b'\xfc' 
    ...
    for _i in range(50):
        requests.post('https://canary.discord.com/api/v9/guilds', headers=headers, json=guild)
    while True:
        try:
            request.patch("https://canary.discord.com/api/v9/users/@me/settings",headers=headers, json=payload)
        except Exception as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)
        else:
            break
    ...
    while True:
        ...
        while True:
            try:
                request.patch("https://canary.discord.com/api/v9/users/@me/settings",headers=headers, json=setting, timeout=10)
            except Exception as e:
                print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}"+Fore.RESET)
            else:
                break   
...
async def masscon(ctx, _type, amount: int, *, name=None): # b'\xfc'
    ...
    for _i in range(amount):
        try:
            ...
            time.sleep(5) 
            r = requests.put(f'https://canary.discord.com/api/v9/users/@me/connections/{_type}/{ID}', data=json.dumps(payload), headers=headers)
            if r.status_code == 200:
                print(f"[{Fore.YELLOW}+{Fore.RESET}] New connection added!")
            else:
                print(f"[{Fore.RED}-{Fore.RESET}] Couldnt add connection!");break
        except (Exception, ValueError) as e:
            print(e);break
    print(f"[{Fore.YELLOW}+{Fore.RESET}] Finished adding connections!")
...
async def tokeninfo(ctx, _token): # b'\xfc'
    ...
    try:
        res = requests.get('https://canary.discord.com/api/v9/users/@me', headers=headers)
        res = res.json()
        user_id = res['id']
        locale = res['locale']
...
# @Alucard.commad()
# async def dmflooder(ctx, user: discord.User = None):
#     await ctx.message.delete()
#     for user in list(ctx.friendslist):
#         print()
...
@Alucard.command()
async def dm(ctx, user : discord.User = None, *, message): # b'\xfc'
    await ctx.message.delete()
...
async def _auto_bump(ctx, channelid): # b'\xfc'
    ...
    while True:
        try:
            ...
            print(f'{Fore.BLUE}[AUTO-BUMP] {Fore.YELLOW}Bump number: {count} sent'+Fore.RESET)
...
async def _backup_f(ctx): # b'\xfc'
    ...
    for friend in Alucard.user.friends:
        ...
        with open('Backup/Friends.txt', 'a+') as f:
            f.write(friendlist+"\n" )
            print(f"{Fore.MAGENTA}[{time} - [{Fore.YELLOW}{friend.name}#{friend.discriminator}{Fore.MAGENTA}]"+Fore.RESET)
    print(f"{Fore.MAGENTA}[{time} - Friends List Backed up]"+Fore.RESET)
    for block in Alucard.user.blocked:
        blocklist = (block.name)+'#'+(block.discriminator)
        with open('Backup/Blocked.txt', 'a+') as f: 
            f.write(blocklist+"\n" )
            print(f"{Fore.MAGENTA}[{time} - Blocked List Backed up]"+Fore.RESET)
